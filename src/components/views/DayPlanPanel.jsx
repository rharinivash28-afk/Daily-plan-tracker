...
// Shows one day's full plan. Used inside the side panel AND the full-screen view.
export default function DayPlanPanel({ dateKey, habits, weekStartsOn, journal, onToggle, onSetValue, onEdit, onDelete, onArchive, onAdd, full }) {
    ...
    <div className="space-y-4">
      ...
      {due.length === 0 ? (
        <div className="py-10 text-center text-ink-muted text-sm">
          ...
          No trackers scheduled this day.
        </div>
      ) : (
        <div className="space-y-2.5">
          {due.map((h) => (
            <HabitCard
              ...
              onToggle={onToggle} onSetValue={onSetValue} onEdit={onEdit} onDelete={onDelete} onArchive={onArchive}
            />
          ))}
        </div>
      )}
      ...
      <Button variant="outline" className="w-full" onClick={onAdd}><Plus size={16} /> Add a tracker</Button>
    </div>
  );
}
